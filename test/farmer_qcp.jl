...
@testset "optimize!: $j" for j in [DSPopt.Legacy, DSPopt.ExtensiveForm] #instances(DSPopt.Methods)
    ...
    if dsp.solve_type == DSPopt.Legacy
        ...
        @test isapprox(primsol[0], [80.0, 120.0, 300.0])
        if dsp.solve_type != DSPopt.Benders
            @test isapprox(primsol[1], [0.0, 0.0, 40.0, 192.0, 6000.0, 1200.0])
            @test isapprox(primsol[2], [0.0, 0.0, 0.0, 120.0, 6000.0, 0.0])
            @test isapprox(primsol[3], [40.0, 0.0, 0.0, 48.0, 4800.0, 0.0])
        end
        @test dualsol == []
        @test isapprox(value(x[1]), 80.0)
        @test isapprox(value(x[2]), 120.0)
        @test isapprox(value(x[3]), 300.0)
    end
